<?php
/**
 * Template Name: Detail Wisata (Custom DB)
 * Digunakan untuk menampilkan detail wisata dari tabel dw_wisata
 */
...
// 2. Query Data
if ($wisata_id > 0) {
    // Pastikan nama tabel benar sesuai prefix wp-config.php
    // Kita gunakan prepare statement untuk keamanan
    $query = $wpdb->prepare("
    ...
    $wisata = $wpdb->get_row($query);
}

// 3. Jika Data Tidak Ditemukan
if (!$wisata) {
    echo '<div class="container mx-auto py-20 px-4 text-center">';
    echo '<div class="text-6xl text-gray-300 mb-4"><i class="fas fa-search-location"></i></div>';
    echo '<h1 class="text-2xl font-bold text-gray-800 mb-2">Wisata Tidak Ditemukan</h1>';
    echo '<p class="text-gray-500 mb-6">Maaf, data wisata yang Anda cari tidak tersedia atau telah dihapus.</p>';
    echo '<a href="'.home_url('/').'" class="bg-primary text-white px-6 py-2 rounded-full font-bold hover:bg-green-700 transition">Kembali ke Beranda</a>';
    echo '</div>';
    get_footer();
    exit;
}
...
// Handle Fasilitas: Bisa string comma-separated atau JSON array tergantung penyimpanan
$fasilitas = [];
if (!empty($wisata->fasilitas)) {
    // Coba decode JSON dulu, kalau gagal anggap comma-separated string
...
$rating = $wisata->rating_avg > 0 ? $wisata->rating_avg : '4.5'; // Fallback rating jika null
...
<div class="container mx-auto px-4 py-8">
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="w-full lg:w-2/3">
            ...
            <!-- Tabs Nav (Optional Style) -->
            <div class="flex gap-6 border-b border-gray-200 mb-6 text-sm font-bold text-gray-500 overflow-x-auto hide-scroll">
                <button class="pb-2 border-b-2 border-primary text-primary whitespace-nowrap">Deskripsi</button>
        ...
        <div class="w-full lg:w-1/3">
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6 sticky top-24">
                ...
                <div class="space-y-4">
                    ...
                    <button class="block w-full bg-gray-100 text-gray-600 text-center py-3 rounded-xl font-bold hover:bg-gray-200 transition">
                        Simpan ke Wishlist
                    </button>
                </div>
            </div>
...
<div class="bg-gray-50 py-10 mt-8 border-t border-gray-200">
    <div class="container mx-auto px-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <?php
            // Query Related (Random 4)
            // Mengambil wisata lain yang aktif dan bukan wisata yang sedang dibuka
            ...
            if($related): foreach($related as $r):
                ...
                $r_link = home_url('/wisata/?id=' . $r->id);
            ?>
            <a href="<?php echo esc_url($r_link); ?>" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group hover:shadow-md transition block">
                ...
                <div class="p-3">
                    ...
                    <p class="text-xs text-gray-500 mb-2 truncate"><i class="fas fa-map-marker-alt text-red-400 mr-1"></i> Lihat Detail</p>

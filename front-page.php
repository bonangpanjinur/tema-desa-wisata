...
// Cek apakah fungsi helper API sudah ada (dari functions.php)
if (function_exists('dw_fetch_api_data')) {
    $data_banner = dw_fetch_api_data($endpoint_banner);
}

$banners = [];
// Normalisasi Data Banner dari API
...
// Fallback Dummy jika API kosong (Agar tampilan tidak rusak saat demo)
if (empty($banners)) {
    $banners = [
        [
...
$data_wisata = function_exists('dw_fetch_api_data') ? dw_fetch_api_data($endpoint_wisata) : [];
$list_wisata = [];
...
// Ambil 4 item saja untuk section populer
$list_wisata = array_slice($list_wisata, 0, 4);

// --- C. GET PRODUK (UMKM) ---
// Tambahkan parameter per_page=10
$endpoint_produk = '/wp-json/dw/v1/produk?per_page=10';
$data_produk = function_exists('dw_fetch_api_data') ? dw_fetch_api_data($endpoint_produk) : [];
$list_produk = [];
...
<!-- =================================================================================
     2. TAMPILAN (VIEW) - MENGGUNAKAN DESAIN REFERENSI ANDA
     ================================================================================= -->
...
<div class="mt-4 px-0 md:px-0">
    ...
    <div class="flex md:grid md:grid-cols-2 overflow-x-auto gap-4 no-scrollbar snap-x md:snap-none pb-4 md:pb-0">
        ...
        <?php foreach ($banners as $index => $banner) : 
            // Mapping Variabel API agar sesuai dengan desain
            $img   = $banner['gambar'] ?? $banner['image_url'] ?? $banner['thumbnail'] ?? 'https://via.placeholder.com/800x400?text=No+Image';
...
<div class="mt-8 mb-8">
    <div class="grid grid-cols-4 md:flex md:justify-center md:gap-12 gap-4 text-center">
        <!-- Link diubah agar menggunakan parameter query string untuk filter di halaman lain jika perlu -->
        <a href="<?php echo home_url('/?dw_type=wisata'); ?>" class="flex flex-col items-center gap-2 group">
        ...
        <a href="<?php echo home_url('/?dw_type=produk'); ?>" class="flex flex-col items-center gap-2 group">
...
<div class="mt-10">
    <div class="flex justify-between items-end mb-4 px-1">
        ...
        <a href="<?php echo home_url('/?dw_type=wisata'); ?>" class="text-emerald-600 text-sm font-bold hover:underline">Lihat Semua <i class="fas fa-arrow-right ml-1"></i></a>
    </div>
    ...
    <div class="flex md:grid md:grid-cols-4 gap-4 overflow-x-auto md:overflow-visible no-scrollbar pb-4 md:pb-0 -mx-4 md:mx-0 px-4 md:px-0">
        ...
        if (!empty($list_wisata)) :
            foreach ($list_wisata as $wisata) :
                // MAPPING DATA API KE VARIABEL TAMPILAN
                // Perhatikan: Key di API mungkin berbeda dengan Database lokal
                $id         = $wisata['id'] ?? 0;
                $judul      = $wisata['nama_wisata'] ?? $wisata['title'] ?? 'Wisata Tanpa Nama';
                // API seringkali mengembalikan lokasi dalam object atau string, sesuaikan
                ...
                // Link Detail (Client side navigation via Parameter)
...
<div class="mt-12 mb-8">
    <div class="flex justify-between items-end mb-4 px-1">
        ...
        <a href="<?php echo home_url('/?dw_type=produk'); ?>" class="text-emerald-600 text-sm font-bold hover:underline">Lihat Semua <i class="fas fa-arrow-right ml-1"></i></a>
    </div>
